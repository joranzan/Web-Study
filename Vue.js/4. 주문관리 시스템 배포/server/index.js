...
app.post("/api/menus", upload.single('file'), async (req, res) => {
	try { //multer로 업로드하면 req안에 file객체를 넘겨받는다.
		console.log(req.file);
		const filename = req.file.filename;
		const result = await pool.query("INSERT INTO menus (name, description, image_src) VALUES(?,?,?)",
			[req.body.name, req.body.description, filename]);
		return res.json({ success: true, message: "메뉴 등록에 성공하였습니다." })
	} catch (error) {
		console.log(error);
