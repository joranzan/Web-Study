        ...
        if(error){
            ...
            fs.readFile(`./logs/${logFiles[0]}`, (err,data) => {
                if(err){
                    ...
                    const parsedFirst = log.split("\r\n");
                    //console.log(parsedFirst);

                    const parsedSecond = parsedFirst.filter(temp => temp!=='');
                    console.log(parsedSecond);

                    const parsedThird = parsedSecond.reduce((acc,cur)=>{
                    ...
                    socket.emit("fromServer", parsedThird);
